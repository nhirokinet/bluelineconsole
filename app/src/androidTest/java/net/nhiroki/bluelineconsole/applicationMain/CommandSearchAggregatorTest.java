...
public class CommandSearchAggregatorTest  extends AndroidTestCase {
    ...
    public void commandSearchAggregatorLeastBasicFunctionTest() {
        ...
        List<CandidateEntry> candidateEntryList = commandSearchAggregator.searchCandidateEntries("test_site_for_test", this.getContext());
        assertEquals(1, candidateEntryList.size());
        // assertEquals("Access Test Site 1", candidateEntryList.get(0).getTitle());

        List<CandidateEntry> candidateEntryList2 = commandSearchAggregator.searchCandidateEntries("test_site", this.getContext());
        assertEquals(2, candidateEntryList2.size());
    }
