...
public class Ping6Activity extends BaseWindowActivity {
    ...
    protected void onResume() {
        ...
        try {
            ProcessBuilder pb = new ProcessBuilder(TARGET_COMMAND_SHORT, host).redirectErrorStream(true);
        ...
        this._readerThread = new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    ...
                    try {
                        int r = Ping6Activity.this._commandIS.read(buf);
                        if (r == -1) {
                            return;
                        }
                        if (r > 0) {
                            Ping6Activity.this.writeOutput(new String(buf, 0, r));
                        }
                    } catch (IOException e) {
                        Ping6Activity.this.writeOutput("\nIOException\n");
                        return;
                    }
    ...
    private void writeOutput(final String s) {
        Thread th = new Thread(new Runnable() {
            @Override
            public void run() {
                Ping6Activity.this.runOnUiThread(new Runnable() {
                    @SuppressLint("SetTextI18n")
                    @Override
                    public void run() {
